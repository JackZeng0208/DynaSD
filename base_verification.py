    ...
    for prompt in tqdm(prompts):
        ...
        if not using_speculative_decoding:
            with torch.no_grad():
                ...
                output = target_model.generate(input_ids, max_length=max_length, temperature=temperature, num_return_sequences=1)
                end_time = time.time()
        else:
            with torch.no_grad():
                start_time = time.time()
                output = target_model.generate(input_ids, max_length=max_length, temperature=temperature, num_return_sequences=1, assistant_model=draft_model)
                end_time = time.time()
