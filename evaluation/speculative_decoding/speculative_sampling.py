...
from .utils import norm_logits, sample, max_fn, greedy_sample
...
def greedy_speculative_sampling_for_training(
    ...
    while prefix.shape[1] < T:
        ...
        if n < prefix_len + gamma - 1:
            ...
            t = greedy_sample(target_model_cache._prob_history[:, n, :])
            resample_count += 1
            target_model_cache.rollback(n+1)
        else:
            # all approx model decoding accepted
            assert n == target_model_cache._prob_history.shape[1] - 1
            t = greedy_sample(target_model_cache._prob_history[:, -1, :])
...
def greedy_speculative_sampling(
    ...
    approx_model_cache = KVCacheModel(approx_model, greedy=True)
    target_model_cache = KVCacheModel(target_model, greedy=True)
...
@torch.no_grad()
def speculative_sampling(prefix : torch.Tensor, 
                         approx_model : torch.nn.Module, 
                         target_model : torch.nn.Module, 
                         max_len : int , 
                         gamma : int = 4,
    ...
    approx_model_cache = KVCacheModel(approx_model, temperature, top_k, top_p, greedy=False)
    target_model_cache = KVCacheModel(target_model, temperature, top_k, top_p, greedy=False)
