...
class RefinedDecisionModel(nn.Module):
    def __init__(self):
        super(RefinedDecisionModel, self).__init__()
    ...
    def forward(self, x):
        ...
        x = self.layer_norm1(x.squeeze(0) + attn_output)
...
class DecisionModelV1_Tinyllama(nn.Module):
    def __init__(self):
        super(DecisionModelV1_Tinyllama, self).__init__()
        self.layers = nn.Sequential(
            nn.Linear(2048, 1024),
            nn.ReLU(),
            nn.Linear(1024, 512),
