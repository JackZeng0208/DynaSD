...
def evaluate_posterior(
    ...
    """
    stats_keys = ['accepted','token','cand_prob','ground_prob','entropy']
    path = '/home/iasl-transformers/EAGLE/eagle/acceptance_eval.csv'
        ...
        for i in range(1, candidates.shape[1]):
            ...
            for j in range(candidates.shape[0]):
                if is_eq[j]:
                    ...
                    draft_distribution = op[i - 1][p_indices[j][i]].clone()
                    if r <= acp:
                        try:
                            with open(path,'a') as file:
                                writer = csv.DictWriter(file,fieldnames= stats_keys)
                                writer.writerow({'accepted':True,'token':tokenizer.decode( x[None]),'cand_prob':qx.item(),'ground_prob':px.item(),'entropy':Categorical(probs = draft_distribution).entropy().item()})
                        except:
                            pass
                        ...
                        try:
                            with open(path,'a') as file:
                                writer = csv.DictWriter(file,fieldnames= stats_keys)
                                writer.writerow({'accepted':False,'token':tokenizer.decode( x[None]),'cand_prob':qx.item(),'ground_prob':px.item(),'entropy':Categorical(probs = draft_distribution).entropy().item()})
                        except:
                            pass
