...
class TreeAttentionSpeculativeDecoding:
    def __init__(self, stats: bool = False):
        self.time_spend_on_draft_model_generation = 0
        self.time_spend_on_target_model_forward = 0
        self.stats = stats
    ...
    def tree_attn_speculative_decoding(self,
                                       input_ids: torch.Tensor,
                                       draft_model: torch.nn.Module,
                                       target_model: torch.nn.Module,
                                       max_len: int,
                                       tree_config: Tuple[int] = (3, 2, 1),
                                       temperature: float = 1,
                                       top_k: int = 0,
                                       top_p: float = 0,
                                       random_seed: int = 1234) -> torch.Tensor:
        ...
        edge_draft_generator = TreeStrategyGeneration(
            draft_model=draft_model,
            use_tree_trunction=False,
            tree_config=tree_config,
        )

        server_verifier = Verification(target_model=target_model)
        ...
        while input_ids.shape[1] < T:
            ...
            target_logits = server_verifier.target_forward(
                draft_tokens, tree_config=draf_tree_config)
            finish_target_forward_time = time.time()
            self.time_spend_on_target_model_forward += finish_target_forward_time - target_forward_time
        ...
        print(f'total time spend on tree attention speculative decoding: {end_time - start_time}')

        token_generate_speed = 0
        # if (input_ids.shape[-1] - seq_len) >= max_len:
        #     token_generate_speed = max_len / (end_time - start_time)
        #     print(f"Token Generation Speed (with tree attention speculative decoding): {token_generate_speed} tokens/s")
        # else:
        #     token_generate_speed = (input_ids.shape[-1] - seq_len) / (end_time - start_time)
        #     print(f"Token Generation Speed (with tree attention speculative decoding): {token_generate_speed} tokens/s")
        token_generate_speed = (input_ids.shape[-1] - seq_len) / (end_time - start_time)
